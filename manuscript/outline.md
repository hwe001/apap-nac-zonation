...
- Fig. 1 — assumed vs measured gradients for the five enzymes. *Exists:
  `figures/fig1_gradients.png` (generated by `code/make_figures.py`).*
- Fig. 2 — NAC protection vs start time, 4 g and 16 g, assumed vs measured
  (the "8-hour rule" figure; PK-calibrated treatment). *Exists:
  `figures/fig2_nac_window.png` (data: `results/nac_window_pk.npz`).*
- Fig. 3 — M_max calibration: pericentral GSH recovery trajectories vs the
  50%-by-12-h criterion. *Exists: `figures/fig3_calibration.png` (data:
  `results/nac_calibration.npz`).*
- Fig. 4 — staggered vs single ingestion: pericentral G(t) and C(t)
  (2 × 2). *Exists: `figures/staggered_patterns.png` (from
  `code/nac_staggered.py`).*
